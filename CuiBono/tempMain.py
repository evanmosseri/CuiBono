...
def legislators(id=None):
    if not(id):
        page = max(int(request.args.get("page")),
                   0) if request.args.get("page") else 0
        num_per_page = int(request.args.get("num_per_page")
                           ) if request.args.get("num_per_page") else 20
        sort = request.args.get("sort") if request.args.get("sort") != None and request.args.get("sort") in vars(Legislator) else ""
        ...
        prev_contribs = add_args(
            request.url,
            num_contribs_per_page=num_contribs_per_page,
            contrib_page=max(contrib_page - 1, 0)
        )
        next_contribs = add_args(
            request.url,
            num_contribs_per_page=num_contribs_per_page,
            contrib_page=contrib_page + 1
        )
...
def bills(id=None, methods=["GET"]):
    ...
    sort = request.args.get("sort") if request.args.get("sort") != None and request.args.get("sort") in vars(Bill) else ""
...
def contributors(id=None):
    ...
    sort = request.args.get("sort") if request.args.get("sort") != None and request.args.get("sort") in vars(Contributor) else ""
    prev_page = add_args(
        request.url,
        num_per_page=num_per_page,
        page=max(page - 1, 0),
    ...
    if not(id):
        return render_template("contributors.html", contributors=db.session.query(Contributor).order_by(sort if sort else id).offset(num_per_page * page).limit(num_per_page), page=page,prev_page=prev_page,next_page=next_page)
    else:
        contrib = db.session.query(Contributor).get(id)
        return render_template("contributor.html", contributor=contrib, page=page,prev_page=prev_page,next_page=next_page,contributions = contrib.contributions[page*num_per_page:num_per_page*(page+1)])
...
def contributions(id=None):
    ...
    sort = request.args.get("sort") if request.args.get("sort") != None and request.args.get("sort") in vars(Contribution) else ""
    prev_page = add_args(
        request.url,
        num_per_page=num_per_page,
        page=max(page - 1, 0),
    ...
    if not(id):
        temp = db.session.query(Contribution).filter(Contribution.legislator != None).order_by(
            sort if sort else id).offset(num_per_page * page).limit(num_per_page)
        return render_template("contributions.html", contributions=temp, prev_page=prev_page,next_page=next_page)
    else:
        contrib = db.session.query(Contribution).get(id)
        return render_template("contribution.html", contribution=contrib)
...
def search(id=None):
    ...
    page = max(int(request.args.get("page")),
               0) if request.args.get("page") else 0
    num_per_page = int(request.args.get("num_per_page")
                       ) if request.args.get("num_per_page") else 10
    ...
    legis = [db.session.query(Legislator).filter(Legislator.first_name.ilike('%' + str('%'.join(c for c in query)) + '%')).offset(page * num_per_page).limit(num_per_page),
             db.session.query(Legislator).filter(Legislator.last_name.ilike(
                 '%' + str('%'.join(c for c in query)) + '%')).offset(page * num_per_page).limit(num_per_page),
             db.session.query(Legislator).filter(Legislator.id.ilike(
                 '%' + str('%'.join(c for c in query)) + '%')).offset(page * num_per_page).limit(num_per_page),
    ...
    contrib = [db.session.query(Contributor).filter(Contributor.name.ilike('%' + str('%'.join(c for c in query)) + '%')).offset(page * num_per_page).limit(num_per_page),
               db.session.query(Contributor).filter(Contributor.id == single_query_int).offset(
                   page * num_per_page).limit(num_per_page),
               db.session.query(Contributor).filter(Contributor.zip.ilike(
                   '%' + str('%'.join(c for c in query)) + '%')).offset(page * num_per_page).limit(num_per_page),
    ...
    billArray = [db.session.query(Bill).filter(Bill.title.ilike('%' + str('%'.join(c for c in query)) + '%')).offset(page * num_per_page).limit(num_per_page),
                 db.session.query(Bill).filter(Bill.id == single_query).offset(
                     page * num_per_page).limit(num_per_page),
                 db.session.query(Bill).filter(Bill.prefix == single_query).offset(
                     page * num_per_page).limit(num_per_page),
                 db.session.query(Bill).filter(Bill.number == single_query_int).offset(
                     page * num_per_page).limit(num_per_page),
    ...
    if single_query_float != -1:
        contributionsArray = [db.session.query(Contribution).filter(
            Contribution.amount == single_query_float).offset(page * num_per_page).limit(num_per_page)]
...
def bill_to_dict(bill):
    if bill is None:
        return None
    bill_temp = {"id": bill.id, "no_count": bill.no_count, "no_votes": bill.no_votes, "yes_count": bill.yes_count, "yes_votes": bill.yes_votes, "prefix": bill.prefix,
                 "number": bill.number, "session": bill.session, "sources": bill.sources, "sponsor_meta": bill.sponsor_meta, "subjects": bill.subjects, "title": bill.title}
    sponsors_list = []
...
def legis_to_dict(legis):
    if legis is None:
        return None
    legis_dict = {"id": legis.id, "filer_id": legis.filer_id, "sources": legis.sources, "offices": legis.offices, "first_name": legis.first_name,
                  "middle_name": legis.middle_name, "last_name": legis.last_name, "district": legis.district, "party": legis.party, "photo_url": legis.photo_url}
    return legis_dict
...
def contributor_to_dict(contra):
    if contra is None:
        return None
    contra_temp = {"id": contra.id, "name": contra.name,
                   "type": contra.type, "zip": contra.zip}
    contribution_list = []
...
def contribution_to_dict(contra):
    if contra is None:
        return None
    contra_temp = {"id": contra.id, "amount": contra.amount, "filer_id": contra.filer_id,
                   "submitted_date": contra.submitted_date, "contributor_id": contra.contributor_id, "legislator_id": contra.legislator_id}
    contributor_list = []
    legislator_list = []

    contra_temp["contributor"] = contra.contributor.name
    if contra.legislator is not None:
        contra_temp["legislator"] = contra.legislator.first_name + \
            " " + contra.legislator.last_name
    else:
